// https://www.codechef.com/OCT15/status/KSPHERES,nemausus
// https://www.codechef.com/viewplaintext/8349392
//author Naresh
#include <algorithm>
#include <bitset>
#include <cassert>
...
const int NCR = 1001;
int ncr[NCR][NCR];
void pascalTriangle() { rep(i, 0, NCR) rep(j, 0, i+1) { if (j==0 || j==i) ncr[i][j] = 1; else ncr[i][j] = (ncr[i-1][j-1] + (ll)ncr[i-1][j]) % MOD; } }
...
int main() {
  ...
  ll sum = 0;
  rep(i, 0, 1000) {
    top[count] = (top[i] * bottom[i])%MOD;
    if (top[count]) {
      sum = (sum + top[count])%MOD;
      bottom[count] = sum;
      ++count;
    }
  }
  rep(len, 2, c+2) {
    ll ans = 0;
    rep(j, 0, count-len+1) {
      bottom[j] = ((j > 0 ? bottom[j-1] : 0) + (bottom[j]*top[j+len-1])%MOD)%MOD;
      ans = bottom[j];
    }
    cout << ans << " ";
  }
