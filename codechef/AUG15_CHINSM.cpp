// https://www.codechef.com/AUG15/status/CHINSM,nemausus
// https://www.codechef.com/viewplaintext/7838635
//author Naresh
#include <algorithm>
#include <bitset>
#include <cassert>
...
int main() {
  rep(i, 1, MAX+1) {
    for(int j = i; j < MAX+1; j += i) d[j].push_back(i);
  }
  int n = si();
  int k = si();
  rep(i, 0, n) arr[i] = si();
  int nearK = -1;
  rep(i, 0, MAX) near[i+1] = -1;
  rep(i, 0, n) {
    if (i==0) {
      ans[0] = 0;
    } else {
      if (arr[i] <= k) {
        ...
        ans[i] = max(ans[i-1], 1 + max(nearK, near[arr[i]]));
      }
    }
    if (arr[i] > k) {
      int num = arr[i]-k;
      auto it = upper_bound(all(d[num]), k);
      for(auto& iter : d[num]) {
        near[iter] = i;
      }
    } else if (arr[i] == k) {
