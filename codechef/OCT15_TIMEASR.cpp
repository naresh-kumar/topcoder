// https://www.codechef.com/OCT15/status/TIMEASR,nemausus
// https://www.codechef.com/viewplaintext/8346566
//author Naresh
#include <algorithm>
#include <bitset>
#include <cassert>
...
int main() {
  ...
  while(tests--) {
    ...
    rep (i, 0, 12) {
      rep (j, 0, 60) {
        double diff = mangle - hangle;
        if (diff > 180.0) {
          diff = 360.0 - diff;
        }
        if (abs(diff-angle) < epsilon) {
          printf("%02d:%02d\n", i, j);
        }
        hangle += 0.5;
        mangle += 6.0;
        if (mangle-hangle > 360.0) {
          mangle -= 360.0;
        }
      }
    }
