// https://www.codechef.com/NOV15/status/KFUNC,nemausus
// https://www.codechef.com/viewplaintext/8704425
//author Naresh
#include <algorithm>
#include <bitset>
#include <cassert>
...
int reduce(ll n) {
  if (n < 10) return n;
  return reduce(sumOfDigits(n));
}

int main() {
  rep(i, 0, 300) pre[i] = reduce(i);
  int tests = si();
  while(tests--) {
    ll a = reduce(sll());
    ll d = reduce(sll());
    ...
    if (i <= r) {
      ll steps = (r-l+1)/sums.size();
      ans *= steps;
      ll left = (r-l+1) - steps * sums.size();
      for (int i = 0; i < left; ++i) ans += sums[i];
    }
    printf("%lld\n", ans);
  }
  return 0;
