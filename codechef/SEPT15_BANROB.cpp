// https://www.codechef.com/SEPT15/status/BANROB,nemausus
// https://www.codechef.com/viewplaintext/8072501
//author Naresh
#include <algorithm>
#include <bitset>
#include <cassert>
...
int main() {
  ...
  while(tests--) {
    ...
    double p;
    cin >> p;
    double seed = billion * pow(p, m);
    double x = m&1 ? (billion + seed)/(1.0+p) : (billion - seed)/(1.0+p);
    double y = billion - x;
    printf("%.6f %.6f\n", x, y);
  }
  return 0;
