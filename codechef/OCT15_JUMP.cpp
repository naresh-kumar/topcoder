// https://www.codechef.com/OCT15/status/JUMP,nemausus
// https://www.codechef.com/viewplaintext/8372282
//author Naresh
#include <algorithm>
#include <bitset>
#include <cassert>
...
const ll MAX_INF = (1LL << 63) - 1;
const int MIN_INF = (1LL << 31);
int MOD = 1E+7 + 7;

ll dp[300000];
bool solved[300000];
ll solve (int n, vll& p, vll& a, vll& h) {
  if (n == 0) return a[0];
  if (solved[n]) return dp[n];
  ll cost = MAX_INF;
  rep(i, 0, n) {
    if (p[i] < p[n]) {
      setmin(cost, a[n] + (h[n]-h[i])*(h[n]-h[i]) + solve(i, p, a, h));
    }
  }
  solved[n] = true;
  dp[n] = cost;
...
int main() {
  int n = si();
  vll p, a, h;
  rep(i, 0, n) p.push_back(si());
  rep(i, 0, n) a.push_back(si());
  rep(i, 0, n) h.push_back(si());
  cout << solve(n-1, p, a, h) << endl;
  return 0;
}
