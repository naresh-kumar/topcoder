...
ll solve(ll a, ll b, ll m, ll x) {
  if (x <= 0) return 0;
  ll mid = (a + b) % MOD;
  if (num[m] <= x) {
    return ((sum[m] * mid) % MOD - b) % MOD;
  }
  return (solve(a, mid, m - 1, x) +
          solve(mid, b, m - 1, x - num[m - 1])) %
         MOD;
}

ll sumall(vi& arr, ll m, ll x) {
  if (x <= 0) return 0;
  ll ans = 0;
  int i = 0;
  while (x > 0) {
    ans = (ans + solve(arr[i], arr[i + 1], m, x)) % MOD;
    x -= num[m];
    ++i;
  }
  return ans;
...
int main() {
  ...
  rep(t, 0, tests) {
    ...
    svector(arr, n);
    arr.push_back(0);
    ll ans = (sumall(arr, m, y) - sumall(arr, m, x-1)) % MOD;
    if (ans < 0) ans += MOD;
    cout << ans << endl;
  }
  return 0;
