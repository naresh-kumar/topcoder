...
int dp[101][101][101];
int ncr[4000][4000];
int solve (int l, int x, int y, int z, int mod = MOD) {
  if (l == 0) return 0;
  int &ans = dp[x][y][z];
  if (ans != -1) return 0;
  ans = 0;
  int count = ncr[x+y+z][z] * ncr[x+y][y];
  if (x == y && y == z) {
    rep(i, 0,
  }
  if (x > 0) ans += solve(l-1, x-1, y, z);
  if (y > 0) ans += solve(l-1, x, y-1, z);
  if (z > 0) ans += solve(l-1, x, y, z-1);
  printf("%d %d %d -> %d\n", x, y, z, ans);
  return ans;
}
