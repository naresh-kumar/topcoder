...
double solve(int k, int n, int m) {
  if (k == 0) return 0.0;
  if (m < 1) return -1.0;// no answer
  double &ans = dp[k][m];
  if (!almost_equal(ans, -2.0, 4)) return ans;
  ans = -1.0;
  rep(i, 0, n) {
    if (cost[i] <= m) {
      double one = solve(k-1, n, m-cost[i]);
      double two = 0.0;
      if (i != lt[k-1][m-cost[i]]) two = value[i];
      if (i == lt[k-1][m-cost[i]]
          && i != ltt[k-1][m-cost[i]])
        two = value[i]/2.0;
      if (one > -1.0 && one + two > ans) {
        ans = one + two;
        lt[k][m] = i;
        ltt[k][m] = lt[k-1][m-cost[i]];
      }
    }
  }
  return ans;
...
int main() {
  ...
  while(k+n+m > 0) {
    ...
    memset(ltt, -1, sizeof(ltt));
    rep(x,0,21) rep(y,0,101)
      dp[x][y] = -2.0;
    rep(i, 0, n) {
      cost[i] = si();
      value[i] = si();
    }
    double ans = solve(k, n, m);
    printf("%.1f\n", max(0.0, ans));
    if (ans > -1.0) pp(k,m);
    printf("\n");
    k = si();
