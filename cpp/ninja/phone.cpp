...
char keymap[11] = "  adgjmptw";
char maxchars[11] = "1333334341";

int getreps(char* digits) {
  if (*digits == '\0') return 0;
  char value = *digits;
  int count = 0;
  while (*digits == value && count < maxchars[value-'0']) {
    ++digits;
    ++count;
  }
  return count;
...
void printall(char* digits, char* solution, int index) {
  if (*digits == '\0') {
    solution[index] = '\0';
    cout << solution << endl;
  }
  int reps = getreps(digits);
  while (reps) {
    solution[index] = keymap[*digits-'0'] + reps - 1;
    printall(digits + reps, solution, index + 1);
    --reps;
  }
}

void phone_perm(char* digits) {
  char* solution = (char*) malloc(strlen(digits) + 1);
  int index = 0;
  printall(digits, solution, index);
}
