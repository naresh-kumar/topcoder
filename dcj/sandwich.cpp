...
int main() {
  ...
  rep(i, range.first, range.second) {
    ll taste = GetTaste(i);
    sum += taste;
    setmax(max_sum_left, sum);
    left.push_back(sum);
  }
  ll sum_last = 0;
  ll max_sum_last = 0;
  if (not IsFirst()) {
    int lastId = MyNodeId() - 1;
    Receive(lastId);
    sum_last = GetLL(lastId);
    max_sum_last = GetLL(lastId);
  }
  if (not IsLast()) {
    int nextId = MyNodeId() + 1;
    PutLL(nextId, sum + sum_last);
    PutLL(nextId, max(max_sum_left + sum_last, max_sum_last));
    Send(nextId);
  }
  ll max_sum = max_sum_last;
  rep(i, 0, left.size()) {
    sum = left[i] + sum_last;
    left[i] = max(max_sum, sum);
    max_sum = left[i];
  }

  sum = 0;
  vll right;
  right.reserve(range.second - range.first);
  ll max_sum_right = 0;
  irep(i, range.second-1, range.first) {
    ll taste = GetTaste(i);
    sum += taste;
    setmax(max_sum_right, sum);
    right.push_back(sum);
  }
  reverse(all(right));
  ll sum_next = 0;
  ...
  if (not IsLast()) {
    int nextId = MyNodeId() + 1;
    Receive(nextId);
    sum_next = GetLL(nextId);
    max_sum_next = GetLL(nextId);
  }
  if (not IsFirst()) {
    int lastId = MyNodeId() - 1;
    PutLL(lastId, sum + sum_next);
    PutLL(lastId, max(max_sum_right + sum_next, max_sum_next));
    Send(lastId);
  }
  max_sum = max_sum_next;
  irep(i, right.size() - 1, 0) {
    sum = right[i] + sum_next;
    right[i] = max(max_sum, sum);
    max_sum = right[i];
  }
  // ---------------------------------------- //
  ll ans = 0;
  rep(i, 0, left.size()) {
    if (i+1 == left.size()) {
      setmax(ans, left[i] + max_sum_next);
    } else {
      setmax(ans, left[i] + right[i+1]);
    }
  }
  PutLL(0, ans);
  Send(0);
