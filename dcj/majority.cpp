...
int main() {
  ...
  PutInt(0, id);
  Send(0);
  // Collect all probable candidates.
  unordered_set<int> ids;
  if (IsFirst()) {
    rep(id, 0, NumberOfNodes()) {
      Receive(id);
      ids.insert(GetInt(id));
    }

    // collect vote count for these candidates.
    rep (id, 0, NumberOfNodes()) {
      PutInt(id, ids.size());
      iter(i, ids) {
        PutInt(id, i);
